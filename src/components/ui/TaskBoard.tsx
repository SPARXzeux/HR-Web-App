  ...
  return (
    <Modal isOpen onClose={onClose} title="Task Details">
      <div className="space-y-5">
        ...
        {/* Title */}
        <div>
          <h3 className={`text-xl font-bold text-slate-900 leading-snug ${task.status === 'done' ? 'line-through opacity-50' : ''}`}>
            {task.title}
          </h3>
          {task.description && (
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
          {STATUS_SEQUENCE.map((s, i) => (
            <React.Fragment key={s}>
              <span className={`px-2.5 py-1 rounded-full border ${task.status === s ? STATUS_STYLES[s] : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                {STATUS_LABELS[s]}
              </span>
              {i < STATUS_SEQUENCE.length - 1 && <ArrowRight className="h-3 w-3 text-slate-300" />}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-200">
          {!readOnly && task.status !== 'done' && (
            <button
              onClick={cycleStatus}
              className="flex items-center gap-1.5 bg-orange-600 hover:bg-orange-700 text-white font-semibold px-4 py-2 rounded-lg text-sm active:scale-97 transition-all shadow-sm"
            >
          ...
          {!readOnly && task.status !== 'todo' && (
            <button
              onClick={handleReset}
              className="flex items-center gap-1.5 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold px-4 py-2 rounded-lg text-sm active:scale-97 transition-all"
            >
          ...
          {canDelete && (
            <button
              ...
              className="flex items-center gap-1.5 bg-white hover:bg-rose-50 border border-rose-200 text-rose-600 font-semibold px-4 py-2 rounded-lg text-sm active:scale-97 transition-all ml-auto"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="flex-1 min-w-[200px] bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-sm focus:border-orange-500 outline-none text-slate-900"
        />
        <select
        ...
        {filtered.map(task => (
          ...
          >
            <div className="flex items-start gap-4">
              {/* Status circle */}
              <div className={`flex-shrink-0 mt-0.5 h-5 w-5 rounded-full border-2 flex items-center justify-center transition-all ${
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-start gap-2 mb-1">
                  <span className={`font-bold text-slate-900 text-sm ${task.status === 'done' ? 'line-through opacity-60' : ''}`}>
                    {task.title}
                  </span>
              ...
              <span className="text-[10px] text-slate-400 font-semibold flex-shrink-0 mt-1 group-hover:text-orange-500 transition-colors">Click to open →</span>
            </div>
          </Card>
        ))}

        {filtered.length === 0 && (
          <div className="text-center py-16 text-slate-400 font-semibold text-sm italic border-2 border-dashed border-slate-200 rounded-xl">
